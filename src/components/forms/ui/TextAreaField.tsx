...
export interface TextAreaFieldProps extends Omit<React.TextareaHTMLAttributes<HTMLTextAreaElement>, 'name'> {
  label: string;
  noLabel?: boolean;
}
...
export default function TextAreaField({
  ...
  const textareaClassName = [TEXTAREA_CLASSES, className].filter(Boolean).join(' ');

  return (
    <div>
      <label htmlFor={textareaId} className={`block leading-6 font-medium ${noLabel ? 'sr-only' : ''}`}>
        {label}
      </label>
      <div className='mt-2'>
        <textarea
